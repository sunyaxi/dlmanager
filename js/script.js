...
function custom_global_settings() {
	...
	var general_settings = gen("基础设置");
	var torrent_settings = gen("BT设置");
	var ftp_settings = gen("FTP设置");
	var http_settings = gen("HTTP(S)设置");
	var metalink_settings = gen("Metalink设置");
...
function changeTime(time) {
	...
	else if(time < 60*60) return (time/60).toFixed(2) + " 分钟";
	else if(time < 60*60*24) return (time/(60*60)).toFixed(2) + " 小时";
	else return (time/(60*60*24)).toFixed(2) + " 天!!";
...
function updateGlobalStatistics(data) {
    ...
    if (data.uploadSpeed > 0 || data.downloadSpeed > 0) {
      ...
      if (data.uploadSpeed > 0) {
        title = "上传 " + changeLength(data.uploadSpeed, "B/s") + " " + title;
      }
      if (data.downloadSpeed > 0) {
        title = "下载 " + changeLength(data.downloadSpeed, "B/s") + " " + title;
      }
    }
